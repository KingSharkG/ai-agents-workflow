...
6. For any `final_path ∈ {execution-trivial, execution-simple, execution-full}`, you MUST dispatch `Task(executor)` (and then `Task(reviewer)` to finalize the cycle) at least once before returning. Returning from an execute path without an Executor dispatch is a protocol violation: no `ai-work.md` / `summary.md` artifacts get produced, the Reviewer never runs, and downstream gates cannot validate. The `guard-chief-orchestrator-stop` SubagentStop hook fail-stops the run in this case. If a dispatch is genuinely impossible (tool failure, missing artifact root), enter degraded-inline mode via the `orchestrator-degraded` skill and escalate via `blocker-escalation-report` — do NOT silently inline the work in your own turn.
