...
**Budget awareness (batch-edit self-check).** Before starting a sequence of Edit calls, count the planned edits. If the count exceeds 8, write a partial-progress note to `<!-- section:implementation -->` in `ai-work.md` FIRST (a minimal `impl-summary` line: `**PARTIAL — 0/N edits completed.** Remaining: [list]`), then proceed with edits in order. If you complete your edits but sense you are approaching your turn limit before running quality gates and writing the implementation report, STOP, update the partial-progress note with the actual completion count, invoke `implementation-report` with `impl-unresolved-issues` listing the remaining edits, and return. The orchestrator can re-dispatch you for the remainder. Never exhaust your turn budget on edits at the expense of the mandatory artifact write — a partial implementation with documented remainder is a valid hand-off; an empty `<!-- section:implementation -->` is a contract violation.

**Produce-artifact-first (MANDATORY before return):** Before emitting your final response, you MUST invoke the `implementation-report` skill and append its output to `<!-- section:implementation -->` in the subtask's `ai-work.md`. Required sub-sections: `impl-metadata`, `impl-summary`, `impl-files-changed`, `impl-tests-run`, `impl-dynamic-skills`, `impl-unresolved-issues`, `impl-project-state`. Ultra-light path: append the compact `impl-ultra` block. **Returning with an empty `<!-- section:implementation -->` block (open/close markers only, no content between them) is a contract violation — the `guard-chief-orchestrator-stop` hook will fail-stop the orchestrator's turn and the cycle will not be reviewable.** This obligation applies to every dispatch, including trivial-path single-subtask runs and the ultra-light variant — there is no execution path on which the section may be left empty.
