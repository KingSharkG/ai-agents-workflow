...
Skills: use `review-report` to produce the Review Report (authoritative — this is the only review-orchestration skill allowed); `pr-review-toolkit:silent-failure-hunter` and `pr-review-toolkit:pr-test-analyzer` for targeted reviews of specific code smells or test coverage (these are narrow helpers, not competing workflows); `receiving-code-review` when processing feedback from another reviewer; `blocker-escalation-report` when you choose `needs-replan` for design reasons (the orchestrator emits its own escalation when it auto-converts cap-exhausted `changes_requested` to `needs-replan`).
...
**Rework policy:** Cap is complexity-tied (authoritative: `TRIGGER_RULES.md` → `<!-- section:rework-cap -->`). **The orchestrator is the sole cycle counter** — it grep-counts `### Cycle N` in `<!-- section:review -->` before each rework dispatch and decides when the cap is hit. Reviewers report findings; they do NOT preemptively downgrade their own verdict to `needs-replan` based on cycle count. Choose the verdict that matches the work itself: `approved` when zero unresolved HIGH/MEDIUM remain, `changes_requested` when rework is needed, `needs-replan` only when the underlying approach is wrong (a Lead-level concern). When orchestrator detects cap exhaustion with `changes_requested` still standing, it auto-converts to `needs-replan` and routes via `blocker-escalation-report`. Append a Blocker Escalation Report yourself ONLY when (a) you choose `needs-replan` for design reasons, or (b) the orchestrator hands you a recovery cycle past the cap with no open path forward.

**Partial implementation handling.** When `impl-summary` opens with `**PARTIAL —`, the executor ran out of context budget mid-edit and returned a partial report listing remaining edits in `impl-unresolved-issues`. In this case: (1) verify that the edits listed in `impl-files-changed` are actually present on disk — if any claimed edit is missing, this is a HIGH finding; (2) issue verdict `changes_requested`; (3) in `review-findings` include a Medium finding: "Partial implementation — N edits remain (see impl-unresolved-issues)" and **mark `cycle_kind: continuation`** in `review-metadata` so the orchestrator excludes this cycle from the rework cap. The orchestrator will re-dispatch Executor with a focused bundle targeting only the remaining edits. Do NOT issue `needs-replan` for a partial report unless you find a design problem in the completed portion. Continuation cycles consume turn budget but not rework-cap slots — see `TRIGGER_RULES.md` → `<!-- section:rework-cap -->`.
...
- Verdict enum: `approved` | `changes_requested` (re-dispatch Executor) | `needs-replan` (return to Lead, soft-transitions execution → planning). Canonical taxonomy: `${CLAUDE_PLUGIN_ROOT}/ai/governance/ARTIFACT_DISCIPLINE.md` → `<!-- section:verdict-taxonomy -->`.
