...
- `hooks/` — Node.js hook scripts wired via `hooks/hooks.json`: `guard-subtask-skeleton` (blocking `Task` PreToolUse), `guard-pre-dispatch-p1` (blocking `Task` PreToolUse), `evaluate-triggers` (`Task` PreToolUse), `guard-agent-reads` (`Read` PreToolUse), `guard-orchestrator-source-writes` (blocking `Edit|Write|Bash` PreToolUse), `validate-artifact-chain` (`Write|Edit` PostToolUse), `validate-dispatch-bundle` (`Write|Edit` PostToolUse), `validate-summary-telemetry` (`Write|Edit` PostToolUse)
