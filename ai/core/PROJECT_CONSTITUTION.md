...
- Every agent must write diagnostics (telemetry line + context manifest subsection) to `<subtask_id>/summary.md` — NOT to `ai-work.md`. Authoritative format and rules: `${CLAUDE_PLUGIN_ROOT}/ai/playbooks/ORCHESTRATION.md` → `<!-- section:context-manifest -->` and `<!-- section:telemetry -->`. Do not restate them elsewhere.
