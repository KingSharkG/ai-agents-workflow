...
- Every agent appending to `ai-work.md` must append a `### <role>` subsection to `<!-- section:context-manifest -->` and one line to `<!-- section:telemetry -->` in that same file. Authoritative format and rules: `${CLAUDE_PLUGIN_ROOT}/ai/playbooks/ORCHESTRATION.md` → `<!-- section:context-manifest -->` and `<!-- section:telemetry -->`. Do not restate them elsewhere.
