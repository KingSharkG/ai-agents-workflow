...
| `BLOCKED` | `execution` (sub-state `phase: blocked`) | Read `blocked_gates` and `pending_user_actions` from state. Workflow gates (`blocked_gates`) are treated as user-waived (resume-orchestrator already obtained confirmation). For `pending_user_actions`: these are external real-world actions — do NOT proceed to the next subtask without re-confirming each action is physically complete. Surface the list and ask the user to confirm before dispatching. Stage stays `execution` throughout. |
...
| `DONE` | `closure` (terminal) | Read `task_summary_path` from state. Print summary and exit. Also used for `phase: answered` (direct-answer tasks) — these have no artifacts to display; print a note and exit. |
