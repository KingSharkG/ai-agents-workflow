...
- The intake skill classified the task as `direct-answer`, `execution-trivial`, or `plan-only` (trivial path) — those flows skip Delivery PM entirely. See `skills/orchestrator-intake/SKILL.md` and `ai/playbooks/ORCHESTRATION.md` → `<!-- section:trivial-flow -->`.
- The task is a `reversal-packet` re-entry (`closure → execution`). Reversals carry the plan delta themselves — do NOT re-run the planner. See `skills/orchestrator-state/references/stage-discipline.md`.
