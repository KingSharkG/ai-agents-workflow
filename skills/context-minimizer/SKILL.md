...
Use this table to decide *what* the bundle should carry. Per-role assembly procedures (cycle-N rework deltas, finding-ID extraction, role-contract block locations) live in `${CLAUDE_PLUGIN_ROOT}/skills/context-minimizer/references/role-bundles.md`. Read the block matching the dispatch's target role. Content is stable across cycles — read once per session, not per dispatch.
...
The full filtering rules, block format, and `pr-lessons-check` boundary live in `${CLAUDE_PLUGIN_ROOT}/skills/context-minimizer/references/pr-lessons-injection.md`. Read on first executor/reviewer bundle per session.
