...
| 9 | needs-replan follow-through (C7): when the `current_subtask`'s `summary.md` carries `review_verdict: "needs-replan"`, the next state write MUST acknowledge it — either transition to `stage: "planning"` (soft execution→planning reopen, with `stage_reopen_count++` and a fresh `stage_history` entry) OR record a `pending_user_actions` / `blocked_gates` entry referencing the replan or reopen decision (soft-cap escalation). Without this, a needs-replan verdict can sit dangling in `stage: "execution"` and the next dispatch trips Phase 3.5 of `pre-task-guard.js` with a cryptic stage-mismatch error far from the actual cause. | **`validate-orchestration-state-write`** |
...
A parallel backstop runs at the Reviewer's SubagentStop: `guard-reviewer-stop` re-verifies that the current subtask's `summary.md` carries a non-`pending` `review_verdict` (one of `approved`, `changes_requested`, `needs-replan`). This closes the loop on the Reviewer's role contract (LAST write = summary.md finalization) — if the Reviewer hits a context limit or errors mid-finalization, the hook blocks the stop with an actionable message rather than letting a stale `pending` verdict propagate to the P2/P4 gates.
