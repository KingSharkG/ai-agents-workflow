...
- **`pr-lessons-check` consultation is MANDATORY** before claiming implementation complete. Invoke the `pr-lessons-check` skill against the diff produced by this subtask BEFORE writing the final `<!-- /section:implementation -->` close. If the consultation surfaces any matched lessons, address them or note them under `impl-unresolved-issues` with rationale. Record the consultation in `impl-dynamic-skills` ("pr-lessons-check — consulted; <N> matches" or "pr-lessons-check — consulted; no matches"). Skipping the consultation is a protocol violation; reviewers will flag it.
