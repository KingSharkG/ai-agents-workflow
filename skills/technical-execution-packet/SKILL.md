...
**Append** to `<!-- section:tep -->` in the subtask's `ai-work.md`. The section placeholder MUST already exist — if absent, raise a Blocker Escalation Report. Also write the diagnostics footer to `<subtask_id>/summary.md` per `${CLAUDE_PLUGIN_ROOT}/skills/orchestrator-telemetry/references/artifact-footer-protocol.md` (role: `lead`).
...
- **`pr-lessons-check` consultation is OPTIONAL but encouraged** during TEP shaping. When the subtask touches an area or pattern that is plausibly covered by a past lesson (auth, schema, error handling, contracts, retries, security), invoke the `pr-lessons-check` skill against the planned scope (target files + expected contract). Surface any high-confidence matches under `tep-risks` so the Executor sees them. Record the consultation in the Lead's context-manifest subsection in `summary.md` (e.g., "pr-lessons-check — consulted; <N> matches surfaced under tep-risks"). The mandatory consultations are at the Executor and Reviewer stages — see their SKILL.md files.
